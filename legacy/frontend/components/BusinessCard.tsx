import { Star, MapPin, Phone, Clock, Bookmark, TrendingUp, Share2 } from 'lucide-react';
      ...
      <CardContent className="p-4">
        <div className="space-y-2">
          <div className="flex items-start justify-between">
            <h3 className={`text-lg font-semibold line-clamp-1 ${textColor}`}>
              {business.name}
            </h3>
          ...
          {shareMessage && (
            <p className="text-sm text-green-600 text-center">{shareMessage}</p>
          )}
        </div>
      </CardContent>

      <CardFooter className="p-4 pt-0">
        <Button onClick={() => onViewDetails(business)} className="w-full">
          View Details
        </Button>
      </CardFooter>
    </Card>
