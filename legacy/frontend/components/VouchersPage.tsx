...
import { availableVouchers, RedeemedVouchers as mockRedeemedVouchers } from '../data/voucherdata'; // Fixed import
...
export function VouchersPage({
  ...
  const copyToClipboardFallback = (text: string) => {
    try {
      ...
      if (successful) {
        ...
        toast.error('Copy failed', { description: 'Please manually copy the voucher code.' });
      }
    } catch (error) {
      toast.error('Copy failed', { description: 'Please manually copy the voucher code.' });
    }
    ...
    return (
      <Card className="hover:shadow-lg transition-shadow" style={{ backgroundColor: cardBgColor, borderColor: borderColor }}>
        <CardHeader className="pb-3">
          ...
          <CardDescription style={{ color: secondaryTextColor }}>{voucher.description}</CardDescription>
        </CardHeader>
  ...
  const RedeemedVoucherCard = ({ redeemedVoucher }: { redeemedVoucher: RedeemedVoucher }) => {
    ...
    const daysLeft = Math.ceil((new Date(redeemedVoucher.expiryDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));

    return (
      <Card className={redeemedVoucher.isUsed || isExpired ? 'opacity-60' : ''} style={{ backgroundColor: cardBgColor, borderColor: borderColor }}>
        <CardHeader className="pb-3">
          <div className="flex items-start justify-between">
            <div className="p-3 rounded-lg" style={{ backgroundColor: `${redeemedVoucher.voucher.color}20` }}>
              <Icon className="w-6 h-6" style={{ color: redeemedVoucher.voucher.color }} />
            </div>
            {redeemedVoucher.isUsed ? (
              <Badge className="bg-green-600 text-white"><CheckCircle2 className="w-3 h-3 mr-1" />Used</Badge>
            ) : isExpired ? (
          ...
          <CardDescription style={{ color: secondaryTextColor }}>{redeemedVoucher.voucher.description}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 md:pl-6" style={{ backgroundColor: bgColor }}>
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button
              ...
              style={{ color: textColor, borderColor: isDarkMode ? '#666666' : borderColor, fontWeight: 500 }}
            >
            ...
            <div>
              ...
              <p style={{ color: secondaryTextColor }}>Redeem vouchers with your loyalty points</p>
            </div>
          </div>
          <Card style={{ backgroundColor: cardBgColor, borderColor: borderColor }}>
            <CardContent className="p-4">
              ...
              <p className="text-2xl text-[#FFA1A3]">{zustandPoints}</p> {/* Display points from Zustand store */}
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'available' | 'my-vouchers')}>
            ...
            {redeemedVouchers.length > 0 ? (
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {redeemedVouchers.map((redeemedVoucher) => (
                  <RedeemedVoucherCard key={redeemedVoucher.id} redeemedVoucher={redeemedVoucher} />
                ))}
              </div>
            ) : (
              <Card style={{ backgroundColor: cardBgColor, borderColor: borderColor }}>
