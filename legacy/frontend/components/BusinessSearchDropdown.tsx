        ...
        >
          {filteredBusinesses.length > 0 ? (
            <>
              ...
              <div className={`px-4 py-2 text-xs text-center border-t ${borderColor} ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>
                Showing top {limit} results
              </div>
            </>
          ) : (
            <div className={`px-4 py-3 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              {searchTerm ? 'No businesses found' : 'Start typing to search...'}
            </div>
          )}
        </div>
      )}
