...
export function Sidebar({ onNavigate, onLogout, currentView, userName, userEmail, avatarUrl }: SidebarProps) {
  const [hoveredItem, setHoveredItem] = useState<string | null>(null);

  const topNavItems = [
    { icon: <Home className="w-5 h-5" />, label: 'Home Page', action: () => onNavigate('map'), view: 'map' },
    { icon: <MessageCircle className="w-5 h-5" />, label: 'Forum', action: () => onNavigate('forum'), view: 'forum' },
    { icon: <User className="w-5 h-5" />, label: 'Profile', action: () => onNavigate('profile'), view: 'profile' },
  ];

  const bottomNavItems = [
    { icon: <Bell className="w-5 h-5" />, label: 'Notifications', action: () => console.log('Notifications') },
    { icon: <HelpCircle className="w-5 h-5" />, label: 'Help', action: () => console.log('Help') },
    { icon: <Settings className="w-5 h-5" />, label: 'Settings', action: () => console.log('Settings') },
    { icon: <LogOut className="w-5 h-5" />, label: 'Logout', action: onLogout },
  ];
  ...
  return (
    <div className="fixed left-0 top-0 h-screen w-16 bg-gray-900 flex flex-col z-50 shadow-xl">
      ...
      {/* Profile Section */}
      <div className="p-3 border-t border-gray-700 flex items-center space-x-3 cursor-pointer"
        title={`${userName || ''}\n${userEmail || ''}`}
